#!/usr/bin/env python3
"""
SocialPerks — Triple Outreach Automatico — NUOVI PARIGI
Sequenza 3 email per nuovi ristoranti Parigi (Gmail SMTP + SQLite tracking)
"""
...
DB_FILE    = BASE_DIR / "outreach_tracking_paris_new.db"
EXCEL_FILE = BASE_DIR / "SocialPerks_Restaurants_Paris_New.xlsx"
LOG_FILE   = BASE_DIR / "outreach_paris_new.log"
...
def run_daily_outreach() -> int:
    log.info("=== Controllo giornaliero outreach NUOVI PARIGI ===")
    conn       = get_conn()
    now        = datetime.now()
    sent_today = [0]
...
def cmd_status():
    ...
    print()
    print("=" * 52)
    print("  📊  STATO OUTREACH — NUOVI PARIGI")
    print("=" * 52)
...
def main():
    ...
    log.info("🚀 SocialPerks Outreach NUOVI PARIGI — avvio")
    init_db()
    import_from_excel()
